...
class simplex_meth:
    ...
    matriz = [] #define a matriz/tableau
    ...
    def calc_min (self):
        ...
        self.flagIlimitado = 0 #flag que identifica que os coeficientes da matriz na coluna da variavel candidata são negativos,ou seja, a solução é ilimitada
        self.flagVB = 0 #identifica que nãa há mais variaveis para entrar na base
            ...
            for x in range(0,self.quantVar): #for para "andar" na função objetivo da ppl
                if(self.matriz[self.quantEqua][x] < 0): #verificação se o custo relativo de cada variavel da função objetivo é < 0,ou seja,se a variavel é candidata ou não
                    ...
                    if(len(varDict)==0): #caso não haja nenhuma elemento nesse dicionario, todos os coeficientes daquela variavel são negativos ou = a 0
                        ...
                        self.flagIlimitado = 1 #<- necessario para sair do loopwhile
                        break
                    ...
                    self.flagVB = 1
                    break
            if(self.flagIlimitado): #se for ilimitada deve-se sair do loopwhile
                break
            if(self.flagVB): #se não tiver mais candidatos deve-se sair do loopwhile
                self.set_var()
                break
